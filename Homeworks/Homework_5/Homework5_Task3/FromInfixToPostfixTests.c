...
{
	char result[9];
	return fromInfixToPostfix("1+2-3+4-5", result) == CORRECT_WORK_CODE && strcmp(result, "12+3-4+5-");
}
...
{
	char result[19];
	return fromInfixToPostfix("1+2*3-0+5/9*6+7-4/8", result) == CORRECT_WORK_CODE && strcmp(result, "123*+0-59/6*+7+48/-");
}

bool bracketsTest(void)
{
	char result[19];
	return fromInfixToPostfix("(1+2)*3-(0+5/9*(6+7))-4/8", result) == CORRECT_WORK_CODE && strcmp(result, "12+3*059/67+*+-48/-");
}
